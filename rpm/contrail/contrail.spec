...
# Install section of contrail-utils package
install -d -m 755 %{buildroot}/usr/share/contrail-utils
# copy files without extenstion
pushd %{buildroot}/usr/share/contrail/
find . -maxdepth 1 -type f -exec /bin/bash -c 'cp "$0" %{buildroot}/usr/share/contrail-utils/${0%.*}' {} \;
popd
# to avoid conflicting with coreutils (/usr/bin/chmod)
mv %{buildroot}/usr/share/contrail-utils/chmod %{buildroot}/usr/share/contrail-utils/chmod.py
