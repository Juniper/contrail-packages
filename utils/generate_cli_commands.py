...
class ContrailCliGenerator(object):
    ...
    def __init__(self, install_dir, base_dir, package_dir):
        ...
        from sandesh_common.vns.constants import ServiceHttpPortMap, ModuleIds, Module2NodeType, UVENodeTypeNames
        for uveNodeType in UVENodeTypeNames.values():
            cli_module = "contrail_"+uveNodeType.split('-')[0]+"_cli"
            self.module_list[cli_module] = []
        ...
        for svc_name in ServiceHttpPortMap:
            ...
            if svc_name in ModuleIds:
                node = UVENodeTypeNames[Module2NodeType[ModuleIds[svc_name]]]
                cli_module = "contrail_"+node+"_cli"
                self.module_to_cli_mapping[svc_name] = cli_module
                self.module_list[cli_module].append(svc_name)
    ...
    def _populate_entry_points_file(self):
        epfile = open(self._install_dir+"/contrail_cli/entry_points.py", "w")
        epfile.write("\nentry_points_dict = { 'ContrailCli' : "+str(self.entry_points_dict["ContrailCli"])+"}\n")
        epfile.close()
        for module_cli in self.entry_points_dict['console_scripts'].keys():
            if module_cli in os.listdir(self._install_dir):
                epfile = open(self._install_dir+"/"+module_cli+"/entry_points.py", "w")
                epfile.write("\nentry_points_dict = {'console_scripts' : "+str(self.entry_points_dict['console_scripts'][module_cli])+"}\n")
                epfile.close()
        return
    #end _populate_entry_points_file
    
    def _populate_command_list_file(self):
        commandfile = open(self._install_dir+"/contrail_cli/commandlist.py", "w")
        commandfile.write("\ncommands_list = "+str(self.commands)+"\n\n")
        commandfile.close()
        for module_cli in self.commands.keys():
            if module_cli in os.listdir(self._install_dir):
                module_cli_pkg = ''.join(x.capitalize() or '_' for x in module_cli.split('_'))
                cmdfile = open(self._install_dir+"/"+module_cli+"/"+module_cli_pkg+"/commandlist.py", "w")
                cmdfile.write("\ncommands_list = "+str(self.commands[module_cli])+"\n")
                cmdfile.close()
        return
    ...
    def _create_entry_points_and_commands(self, mapping_files):
        ...
        for svc_name in ServiceHttpPortMap:
            ...
            console_script_list = []
            module_cli = self.module_to_cli_mapping[svc_name]
            module_cli_pkg = ''.join(x.capitalize() or '_' for x in module_cli.split('_'))
            console_script_list.append("{0} = {1}.main:{2}".format(console_command,\
                module_cli_pkg, module_cli))
            self.entry_points_dict['console_scripts'][module_cli].append(console_script_list)
    
            self.commands[module_cli][console_command] = list_commands
    ...
    def _create_bash_completion_script(self):
        ...
        for ep in self.entry_points_dict['console_scripts'].values():
            ...
            module_cli = self.module_to_cli_mapping[cmds[:cmds.find("-cli")]]
            completer = CompleteCommand(cmds, self.commands[module_cli][cmds])
