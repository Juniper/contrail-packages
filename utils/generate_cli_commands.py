...
class ContrailCliGenerator(object):
    ...
    def __init__(self, install_dir, base_dir):
        ...
        for cli_module in cli_modules:
            ...
            module = cli_module.split('_')[1]
            key = supervisor_files_dir+"supervisord_"+ module + "_files"
            for dirpath, dirname, files in os.walk(key):
                for name in files:
                    if name.lower().endswith(extn):
                        name = name[:-len(extn)]
                        self.module_to_cli_mapping[name] = cli_module
    #end __init__
