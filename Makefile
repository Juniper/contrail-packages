...
.PHONY: all rpm dep contrail-rpms

dep: $(DEPENDENCIES)
rpm: $(PACKAGES) contrail-rpms
...
contrail-rpms:
ifeq ($(CONTRAIL_BUILD_FROM_SOURCE),true)
	rpmbuild --short-circuit --noclean -bb --with testdepsonly $(RPMBUILD_FLAGS) $(SPEC_DIR)/contrail/contrail.spec
endif
